    ...
    try {
        upload.single('archivo')(req, res, async (err) => {
            ...
            for (let elemento of elementos) {
                ...
                createRecord(area,'prestamoes', idprestamo, adminId, prestamoEspecial.clientes_documento, clienteExists.nombre, idelemento, elementoEncontrado.descripcion, cantidad, observaciones, 'actual', 'PRESTAMO ESPECIAL ELEMENTO', archivo); 
            }
...
// Duplicar préstamo especial
const duplicateLoanEspecial = async (req, res) => {
    const { idprestamo } = req.params;
    const { area } = req.user;
    try {
        // Buscar el préstamo original
        const originalLoan = await PrestamoEspecial.findOne({ where: { idprestamo, estado: 'actual', areas_idarea: area } });
        if (!originalLoan) {
            return res.status(404).json({ mensaje: 'Préstamo especial original no encontrado' });
        }
        // Crear el nuevo préstamo especial
        const newLoan = await PrestamoEspecial.create({
            clientes_documento: originalLoan.clientes_documento,
            estado: 'actual',
            areas_idarea: originalLoan.areas_idarea
        });
        // Buscar los elementos asociados al préstamo original
        const elementosOriginales = await ElementoHasPrestamoEspecial.findAll({ where: { prestamosespeciales_idprestamo: idprestamo, estado: 'actual' } });
        // Copiar cada elemento al nuevo préstamo especial
        for (const elem of elementosOriginales) {
            await ElementoHasPrestamoEspecial.create({
                elementos_idelemento: elem.elementos_idelemento,
                prestamosespeciales_idprestamo: newLoan.idprestamo,
        ...
        return res.status(201).json({ mensaje: 'Préstamo especial duplicado exitosamente', idprestamo: newLoan.idprestamo });
    } catch (error) {
        console.error('Error al duplicar préstamo especial:', error);
        return res.status(500).json({ mensaje: 'Error al duplicar el préstamo especial', error });
    }
};

export { findLoanElements, createPrestamoEspecial, getAllLoanElements, addOrUpdate, getAllLoanElementsTotal, prestamosesManana, duplicateLoanEspecial };