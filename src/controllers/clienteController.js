...
const createClient = async (req, res) => {
    try {
        ...
        upload.single('foto')(req, res, async (err) => {
            ...
            // Corregir areas_idarea si viene como string vacía, 'null', undefined o null
            if (
                req.body.areas_idarea === '' ||
                req.body.areas_idarea === 'null' ||
                req.body.areas_idarea === undefined ||
                req.body.areas_idarea === null
            ) {
                req.body.areas_idarea = null;
            }
...
const getInfoClient = async (req, res) => {
    try {
        ...
        if (user) {
            // Forzar que correo sea string
            const plainUser = user.toJSON();
            plainUser.correo = typeof plainUser.correo === 'string' ? plainUser.correo : '';
            res.json(plainUser);
        } else {
            res.status(404).json({ mensaje: 'Cliente no encontrado' });
        }
