...
const createAdmin = async (req, res) => {
    try {
        ...
        if(!adminExisting) { 
            // Validar y limpiar los datos del body
            const cleanBody = { ...req.body };
            
            // Asegurar que el correo sea un string
            if (cleanBody.correo) {
                if (typeof cleanBody.correo === 'object') {
                    // Si es un objeto o array, tomar el primer valor válido
                    if (Array.isArray(cleanBody.correo)) {
                        cleanBody.correo = cleanBody.correo[0] || '';
                    } else {
                        cleanBody.correo = cleanBody.correo.toString();
                    }
                }
                cleanBody.correo = cleanBody.correo.toString().trim();
            }

            // Validar formato de correo
            if (cleanBody.correo && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cleanBody.correo)) {
                return res.status(400).json({ mensaje: 'Formato de correo inválido' });
            }

            if (cleanBody.contrasena !== '') {
                cleanBody.contrasena = await bcrypt.hash(cleanBody.contrasena, 10);
            }

            if (area == 0) {
                await Administrador.create({...cleanBody, areas_idarea: areaFormSupervisor});
            } else {
                await Administrador.create({...cleanBody, areas_idarea: area});
            }
...
const updateAdmin = async (req, res) => {
    try {
        ...
        if (area == 0) {
            [updated] = await Administrador.update(cleanBody, {
